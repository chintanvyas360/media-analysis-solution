...
class Browse extends Component {
	...
	render() {
		var media_cards = this.state.results.map((item, key) => {
			return (
		...
		var page_numbers = pages.map((page, key) => {
			return (
				<PaginationItem key={key} active={this.state.current_page === { page }.page}>
			...
			<Container>
				...
				<div className="browse-header">
					...
					<div className="pagination-container">
						<Pagination size="sm" className="mb-0 pt-2 pb-2 justify-content-end">
							{page_numbers}
						</Pagination>
						<h6 className="text-secondary viewing-results">Viewing {((this.state.current_page - 1) * 30) + media_cards.length} of {this.state.result_count} results</h6>
					</div>
				</div>
				<div>
					<Container>
